...
public class GoogleDriveCloudProvider: CloudProvider {
	...
	public func createFolder(at cloudPath: CloudPath) -> Promise<Void> {
		...
		return Promise<Void>(on: .global()) { fulfill, reject in
			let parentIdentifier = try awaitPromise(self.resolveParentPath(for: cloudPath))
			do {
				_ = try awaitPromise(self.getFirstIdentifier(forItemWithName: foldername, itemType: .folder, inFolderWithId: parentIdentifier))
				reject(CloudProviderError.itemAlreadyExists)
			} catch CloudProviderError.itemNotFound {
				_ = try awaitPromise(self.createFolder(at: cloudPath, withParentIdentifier: parentIdentifier))
				fulfill(())
	...
	private func traverseThroughPathToFolder(from startCloudPath: CloudPath, to endCloudPath: CloudPath, withStartIdentifier startIdentifier: String) -> Promise<String> {
		...
		return Promise(on: .global()) { fulfill, _ in
			for i in startIndex ..< endIndex {
				...
				parentIdentifier = try awaitPromise(self.getFirstIdentifier(forItemWithName: itemName, itemType: .folder, inFolderWithId: parentIdentifier))
